...
public class UpdateClientUseCaseTests : UseCaseTestBase<UpdateClientUseCase>
{
    ...
    private readonly UpdateClientModel _inputModel;
    ...
    public sealed class Success_Setup : UpdateClientUseCaseTests, IAsyncLifetime
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                Result = await subject.Execute(ProcessRequest<UpdateClientModel>.From(_inputModel, _ct));
            });
        }
    ...
    public sealed class Success : IClassFixture<Success_Setup>
    {
        ...
        [Fact]
        public void Result_ClientId_MatchesUpdatedClient()
        {
            var response = (ProcessResponse<ClientDto>)_fixture.Result!;
            Assert.Equal(_fixture._clientId, response.Result!.ClientId);
        }
    }

    public sealed class NotFound_Setup : UpdateClientUseCaseTests, IAsyncLifetime
    {
