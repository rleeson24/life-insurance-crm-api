...
public class CreateSecondaryEnrollmentUseCaseTests : UseCaseTestBase<CreateSecondaryEnrollmentUseCase>
{
    ...
    private readonly CreateSecondaryEnrollmentModel _inputModel;
    private readonly SecondaryEnrollment _createdEnrollment;
    ...
    public CreateSecondaryEnrollmentUseCaseTests()
    {
        _tenantId = CreateGuid();
        _userId = CreateGuid();
        _clientId = CreateGuid();
        _now = CreateTimestamp();
        _inputModel = Create<CreateSecondaryEnrollmentModel>() with { ClientId = _clientId };
        var enrollmentId = CreateGuid();
        _createdEnrollment = TestFixture.CreateSecondaryEnrollment(enrollmentId, _clientId, _tenantId, _userId, _now);
    }

    protected override CreateSecondaryEnrollmentUseCase BuildSubject() =>
        new(
            ActorTracker.Object,
            NowProvider.Object,
    ...
    public sealed class Success_Setup : CreateSecondaryEnrollmentUseCaseTests, IAsyncLifetime
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                Result = await subject.Execute(ProcessRequest<CreateSecondaryEnrollmentModel>.From(_inputModel, _ct));
            });
        }
    ...
    public sealed class Success : IClassFixture<Success_Setup>
    {
        ...
        {
            var response = (ProcessResponse<SecondaryEnrollmentDto>)_fixture.Result!;
            Assert.Equal(_fixture._createdEnrollment.SecondaryEnrollmentId, response.Result!.SecondaryEnrollmentId);
        }
    }

    public sealed class ClientNotFound_Setup : CreateSecondaryEnrollmentUseCaseTests, IAsyncLifetime
    {
