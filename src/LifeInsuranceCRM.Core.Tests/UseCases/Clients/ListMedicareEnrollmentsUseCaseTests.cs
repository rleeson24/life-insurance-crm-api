...
public class ListMedicareEnrollmentsUseCaseTests : UseCaseTestBase<ListMedicareEnrollmentsUseCase>
{
    private readonly Guid _tenantId;
    private readonly Guid _userId;
    private readonly Guid _clientId;
    private readonly CancellationToken _ct = CancellationToken.None;
    private readonly ListMedicareEnrollmentsRequest _request;
    ...
    public ListMedicareEnrollmentsUseCaseTests()
    {
        _tenantId = CreateGuid();
        _userId = CreateGuid();
        _clientId = CreateGuid();
        _request = new ListMedicareEnrollmentsRequest { ClientId = _clientId };
    }

    protected override ListMedicareEnrollmentsUseCase BuildSubject() =>
        new(
            ActorTracker.Object,
    ...
    public sealed class Success_Setup : ListMedicareEnrollmentsUseCaseTests, IAsyncLifetime
    {
        public Success_Setup()
        {
            var now = CreateTimestamp();
            var enrollmentId = CreateGuid();
            var enrollments = new[] { TestFixture.CreateMedicareEnrollment(enrollmentId, _clientId, _tenantId, _userId, now) };
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                Result = await subject.Execute(ProcessRequest<ListMedicareEnrollmentsRequest>.From(_request, _ct));
            });
        }
    ...
    public sealed class Success : IClassFixture<Success_Setup>
    {
        ...
        {
            var response = (ProcessResponse<IReadOnlyList<MedicareEnrollmentDto>>)_fixture.Result!;
    ...
    public sealed class ClientNotFound_Setup : ListMedicareEnrollmentsUseCaseTests, IAsyncLifetime
    {
