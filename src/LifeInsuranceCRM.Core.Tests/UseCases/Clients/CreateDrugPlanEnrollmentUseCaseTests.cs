...
public class CreateDrugPlanEnrollmentUseCaseTests : UseCaseTestBase<CreateDrugPlanEnrollmentUseCase>
{
    ...
    private readonly CreateDrugPlanEnrollmentModel _inputModel;
    private readonly DrugPlanEnrollment _createdEnrollment;
    ...
    public CreateDrugPlanEnrollmentUseCaseTests()
    {
        _tenantId = CreateGuid();
        _userId = CreateGuid();
        _clientId = CreateGuid();
        _now = CreateTimestamp();
        _inputModel = Create<CreateDrugPlanEnrollmentModel>() with { ClientId = _clientId };
        var enrollmentId = CreateGuid();
        _createdEnrollment = TestFixture.CreateDrugPlanEnrollment(enrollmentId, _clientId, _tenantId, _userId, _now);
    }

    protected override CreateDrugPlanEnrollmentUseCase BuildSubject() =>
        new(
            ActorTracker.Object,
            NowProvider.Object,
    ...
    public sealed class Success_Setup : CreateDrugPlanEnrollmentUseCaseTests, IAsyncLifetime
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                Result = await subject.Execute(ProcessRequest<CreateDrugPlanEnrollmentModel>.From(_inputModel, _ct));
            });
        }
    ...
    public sealed class Success : IClassFixture<Success_Setup>
    {
        ...
        {
            var response = (ProcessResponse<DrugPlanEnrollmentDto>)_fixture.Result!;
            Assert.Equal(_fixture._createdEnrollment.DrugPlanEnrollmentId, response.Result!.DrugPlanEnrollmentId);
        }
    }

    public sealed class ClientNotFound_Setup : CreateDrugPlanEnrollmentUseCaseTests, IAsyncLifetime
    {
