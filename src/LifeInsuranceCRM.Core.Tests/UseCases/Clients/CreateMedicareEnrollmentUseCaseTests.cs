...
public class CreateMedicareEnrollmentUseCaseTests : UseCaseTestBase<CreateMedicareEnrollmentUseCase>
{
    ...
    private readonly CreateMedicareEnrollmentModel _inputModel;
    private readonly MedicareEnrollment _createdEnrollment;
    ...
    public CreateMedicareEnrollmentUseCaseTests()
    {
        _tenantId = CreateGuid();
        _userId = CreateGuid();
        _clientId = CreateGuid();
        _now = CreateTimestamp();
        _inputModel = Create<CreateMedicareEnrollmentModel>() with { ClientId = _clientId };
        var enrollmentId = CreateGuid();
        _createdEnrollment = TestFixture.CreateMedicareEnrollment(enrollmentId, _clientId, _tenantId, _userId, _now);
    }

    protected override CreateMedicareEnrollmentUseCase BuildSubject() =>
        new(
            ActorTracker.Object,
            NowProvider.Object,
    ...
    public sealed class Success_Setup : CreateMedicareEnrollmentUseCaseTests, IAsyncLifetime
    {
        ...
        public async Task InitializeAsync()
        {
            ...
            {
                Result = await subject.Execute(ProcessRequest<CreateMedicareEnrollmentModel>.From(_inputModel, _ct));
            });
        }
    ...
    public sealed class Success : IClassFixture<Success_Setup>
    {
        ...
        {
            var response = (ProcessResponse<MedicareEnrollmentDto>)_fixture.Result!;
            Assert.Equal(_fixture._createdEnrollment.MedicareEnrollmentId, response.Result!.MedicareEnrollmentId);
        }
    }

    public sealed class ClientNotFound_Setup : CreateMedicareEnrollmentUseCaseTests, IAsyncLifetime
    {
